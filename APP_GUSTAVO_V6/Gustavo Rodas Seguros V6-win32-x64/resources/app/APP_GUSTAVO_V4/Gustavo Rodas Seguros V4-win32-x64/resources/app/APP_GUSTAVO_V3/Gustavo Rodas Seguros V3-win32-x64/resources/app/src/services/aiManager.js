...
async function _callClaude(prompt, fileBase64) {
    ...
    // Configuración Técnica - Modelo 2026
    // NOTA: El usuario solicitó mantener "claude-sonnet-4-6" como registro histórico por si falla la nueva versión.
    // const model = "claude-sonnet-4-6"; 
    const model = "claude-3-5-sonnet-20241022";

    try {
        console.log(`📡[PROBANDO CLAUDE] Configuración - Modelo: ${model} `);
        ...
        if (response.status !== 200) {
            ...
            throw new Error(`Claude Error(${response.status}): ${JSON.stringify(errorBody)} `);
        }
        ...
        console.error(`❌[CLAUDE CRITICAL]: `, err.message);
        throw err; // El orquestador _callAI se encargará de saltar a Gemini
    }
}
