...
const PolicyManager = () => {
    ...
    const filteredPolicies = useMemo(() => {
        ...
        return policies
            .filter(p => {
                // 1. Búsqueda Robusta (Bypass de filtros si hay texto)
                ...
                const matchesSearch = terms.length === 0 || terms.every(term => searchBlob.includes(term));

                // 2. Filtros de Categoría (Solo se aplican si NO hay búsqueda activa para facilitar encontrar clientes específicos)
                const isSearching = terms.length > 0;
                const matchesRisk = isSearching || filterRisk === 'All' || normalizeRisk(p.riskType) === filterRisk;
                const matchesCompany = isSearching || filterCompany === 'All' || abbreviateCompany(p.company) === filterCompany;

                // 3. Filtro de Adjuntos (Bypass si hay búsqueda)
                const hasFile = !!(p.fileUrl || p.fileBase64 || (p.attachments && p.attachments.length > 0));
                const matchesAttachment = isSearching || filterAttachment === 'All' || (filterAttachment === 'WithFile' ? hasFile : !hasFile);
                const matchesMissingFilter = isSearching || !showOnlyMissingFiles || !hasFile;

                // 4. ESTADO: Activa vs Vencida vs Anulada (Este filtro se mantiene siempre por seguridad)
                ...
                if (p.clientName?.toUpperCase().includes('GIL') || p.clientName?.toUpperCase().includes('OLIVARES')) {
                    console.log(`[V12 DEBUG] ${p.clientName}:`, {
                        id: p.id || 'N/A',
                        visible: result,
                        razones: {
                            matchesSearch,
                            matchesRisk: isSearching ? "BYPASS" : matchesRisk,
                            matchesCompany: isSearching ? "BYPASS" : matchesCompany,
                            matchesStatus,
                            matchesAttachment
                        },
                        detalles: {
                            ...
                            cancelled,
                            risk: p.riskType,
                            normRisk: normalizeRisk(p.riskType),
                            company: p.company,
                            abbrCompany: abbreviateCompany(p.company),
                            hasFile,
                            attachments: p.attachments?.length || 0
                        },
                        filtrosActuales: { searchTerm, filterRisk, filterCompany, filterStatus }
                    });
                }
