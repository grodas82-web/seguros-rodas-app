...
export const AppProvider = ({ children }) => {
    ...
    const fixInboundInvoice = (inv) => {
        if (inv.cuit === '23294824979') {
            ...
            if (targetComp && targetComp.cuit && targetComp.cuit !== '23294824979') {
                return { ...inv, cuit: targetComp.cuit };
            }
        }
        return inv;
    };
    ...
    const addInvoice = async (invoice) => {
        if (checkDuplicate(invoice)) {
            console.warn("Factura duplicada detectada en invoices");
            return false;
        }
    ...
    const processInvoiceFile = async (file, preLoadedBase64 = null, onProgress = null) => {
        ...
        let usageMetadata = {};
        try {
            if (onProgress) onProgress('Analizando con IA (Failover Activo)...', 30);
            const aiResult = await analyzeInvoice(base64Data);

            if (!aiResult || !aiResult.data) throw new Error("La IA retornó una respuesta vacía.");
    ...
    const updatePolicy = async (id, data) => {
        try {
            ...
            await updateDoc(doc(db, 'policies', id), cleanData);
