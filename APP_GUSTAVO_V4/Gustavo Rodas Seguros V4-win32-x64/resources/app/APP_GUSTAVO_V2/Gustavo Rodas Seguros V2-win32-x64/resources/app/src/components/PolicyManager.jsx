...
const PolicyManager = () => {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    if (result.status === 'success') {
                        results.push({
                            ...result.data,
                            status: 'AI Multi-Upload'
                        });
                        successCount++;
                        successFileNames.push(file.name);
                    } else {
            ...
            if (results.length > 0) {
                setProgress({ message: 'Guardando y unificando registros...', percent: 95 });
                const finalCount = await bulkAddPolicies(results);

                let msg = `✅ RESULTADO DE CARGA IA\n`;
                msg += `━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
                msg += `✅ ÉXITOS: ${successCount} de ${files.length} archivos\n`;
                msg += `📋 Pólizas guardadas: ${finalCount}\n\n`;

                if (successFileNames.length > 0) {
                    msg += `ARCHIVOS OK:\n`;
                    successFileNames.forEach(n => { msg += `  ✓ ${n}\n`; });
                }
